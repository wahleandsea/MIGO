...
- 服务器负责的规则（网页里的按钮不能绕过）：
  ...
  - 司机接单前看不到乘客姓名。
- 目前距离仍由用户在页面上拖动输入，服务器只能限制范围；真实路线距离待接入路线计算服务。

## 地图（Waze 官方集成）

已接入 Waze 的两个**官方**能力，不需要申请 API Key：

- **Waze Live Map 嵌入**：乘客端叫车页顶部显示米兰市中心的 Waze 实时地图（路况、拥堵实时更新）。
  使用官方 iframe（`embed.waze.com/iframe`，参数 `lat` / `lon` / `zoom` / `pin`），代码见 `src/app.js` 的 `wazeEmbedUrl()`。
- **Waze Deep Link 导航**：乘客端可一键“在 Waze 查看上车点 / 导航到目的地”（输入框内容实时同步）；
  司机接单后行程卡里有“用 Waze 导航到上车点 / 目的地”；订单详情里也可“在 Waze 中查看路线”。
  链接统一走 `https://waze.com/ul?q=地址&navigate=yes&vehicle_type=taxi`，手机上直接打开 Waze App 开始导航，电脑上打开 Waze 网页。

**重要限制**：Waze **不提供公开的路线/距离/ETA 计算 API**（官方文档明确说明；逆向的“Waze 路线接口”不受支持，生产环境不要用）。所以：

- 页面上的“预计距离”和路线报价仍由滑块模拟，服务端也无法按真实路线算距离；
- 要按真实路线计价，需要另接一个路线计算服务：推荐开源免费的 **OSRM** 公共演示接口（无需 Key，适合练手），或 **Google Directions / Mapbox / TomTom**（需注册 API Key）。确定用哪个后，可以继续接上。
...
- 地图：Waze 实时路况展示（官方 iframe）+ 一键 Waze 导航跳转（Deep Link）。
- 路线模拟、不同路线报价与公里阶梯计价。
